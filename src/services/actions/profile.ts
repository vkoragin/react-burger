...
export const updateUser =
  (userData: TUpdateUserData) => async (dispatch: AppDispatch | AppThunk) => {
    dispatch({
      type: GET_USER,
    });
    ...
    try {
      ...
      if (err.response?.status === 403) dispatch(refreshToken(updateUser));
      return false;
    }
  };

export const getUser = () => async (dispatch: AppDispatch | AppThunk) => {
  dispatch({
    type: GET_USER,
  });
